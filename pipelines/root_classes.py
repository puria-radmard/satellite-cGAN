  ...
  def group_operation(self, group):
      ...
      try:
        ...
        if not isinstance(output_image, list):
          raise TypeError("Please return output image(s) as a list in operation method")
...
class OperationPipeline:
  ...
  def show_overlay(self, root, image_id, overlay_band, underlay_bands = ["B2", "B3", "B4"]):
    overlay_image = visualise_bands(root, image_id = image_id, bands = overlay_band, show = False)
    underlay_image = visualise_bands(root, image_id = image_id, bands = underlay_bands, show = False)
...
class StatisticalAnalysisOperation:
  ...
  def operation(self, dependant_variable_dict, aggregation_list):
    ...
    independant_variable_dict = {
        original_path: {
            band: read_raster(self.replace_band_name(original_path, band))[0] for band in self.input_bands
    ...
    for agg_level in aggregation_list:
      ...
      for path, arr in dep.items():   
        dep[path] = cv2.resize(dep[path], (0, 0), fx = 1/agg_level, fy = 1/agg_level)

      for path, arr in dep.items():
        ind = copy.deepcopy(independant_variable_dict)
 
        for band, arr1 in ind[path].items():
          ind[path][band] = cv2.resize(arr1, (0, 0), fx = 1/agg_level, fy = 1/agg_level)

        try:
          aggregated_result = self.stats_op(ind[path], dep[path])
          row = {
              "city": [path.split("/")[-1].split("--")[1] for _ in range(len(list(aggregated_result.values())[0]))],
              "hash": [path.split("/")[-1].split("--")[0] for _ in range(len(list(aggregated_result.values())[0]))],
              "datetime": ["--".join(path.split("/")[-1].split(".")[0].split("--")[-2:]) for _ in range(len(list(aggregated_result.values())[0]))],
              "agg_value": [agg_level for _ in range(len(list(aggregated_result.values())[0]))]
          }
  ...
  def __call__(self, root, aggregation_list = [1]):
    ...
    self.results = pd.DataFrame(
        columns = self.core_col_names + self.stats_col_names
    )
