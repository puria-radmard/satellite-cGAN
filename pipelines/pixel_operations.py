...
class NDBIOperation(Operation):
  ...
  def operation(self, rasters):
    return [(rasters["B6"][0] - rasters["B5"][0]) / (rasters["B6"][0] + rasters["B5"][0])], None
...
class NDWIOperation(Operation):
  ...
  def operation(self, rasters):
    return [(rasters["B3"][0] - rasters["B5"][0]) / (rasters["B3"][0] + rasters["B5"][0])], None
...
class NDVIOperation(Operation):
  ...
  def operation(self, rasters):
    return [(rasters["B5"][0] - rasters["B4"][0]) / (rasters["B5"][0] + rasters["B4"][0])], None
...
class LSTOperation(Operation):
  ...
  def operation(self, rasters):
    ...
    TOA_raster = self.variables["M_L"] * rasters["B10"][0] + self.variables["A_L"] - 0.29
    BT_raster  = (self.variables["K2"] / np.log(1 + self.variables["K1"]/TOA_raster)) - 273.15
    VEP_raster = ((rasters["NDVI"][0] - self.variables["NDVIs"]) / (self.variables["NDVIv"] - self.variables["NDVIs"]))**2
    ...
    LST_raster = BT_raster / (1 + np.log(EPS_raster) * self.variables["CONST_A"] * BT_raster / self.variables["CONST_B"])
