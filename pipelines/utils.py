...
def get_property_path(group: Dict[str, str], prop_name: str):
  ...
  prop_path = os.path.join("/".join(list(group.values())[0].split("/")[:-1]), f"{group_name}.{prop_name}.tif")
  return prop_path
...
def visualise_bands(root, image_id, bands, show = False, viz_factors = [1, 1, 1]):
  ...
  if show:
    ...
    axs[0].hist([band_originals[j].flatten() for j in range(len(bands))], label=bands, alpha=0.5, bins = 30)
...
def get_metadata(group: Dict[str, str]):
  ...
  group_name = list(group.values())[0].split(".")[-3].split("/")[-1]
  metadata_json_path = os.path.join("/".join(list(group.values())[0].split("/")[:-1]), f"{group_name}.METADATA.json")
